...
object Job1 {
  ...
  def main(args: Array[String]): Unit = args match {
    ...
    case Array("optimizedWithoutPartitioning") => optimizedWithoutPartitioning()
    ...
    case _ => sys.error("""
        ...
        |    optimizedWithoutPartitioning - runs the optimized version of the job without partitioning
        |    optimized                    - runs the complete optimized version of the job
        |""".stripMargin)
  }
  ...
  private def basic(): Unit = {
    ...
    val businessesStates = data.metadataRdd.map(b => b._3 -> toState(b._2)) // [(gmap_id, state)*]
  ...
  private def optimizedWithoutPartitioning(): Unit = {
    implicit val spark: SparkSession = SparkSession.builder.appName("Job1 Optimized without partitioning").getOrCreate()
    val data = new DataResolver()
    import spark.sqlContext.implicits._ // needed to save as CSV, see `toDF` method
    ...
    outcome
      ...
      .option("header", "true")
      .mode(SaveMode.Overwrite)
      .save(Commons.getDatasetPath("remote", s"${Config.outputDirPath}/job1-optimized-without-partitioning-output"))
  }
